...
function ProductInfo() {
  ...
  return productShoesData ? (
    <div className="flex md:flex-row flex-col bg-white dark:bg-slate-950 transition duration-300">
      {/* Product Image */}
      <div className=" imaged-div w-2/4">
        <img
          src={productShoesData.image}
          alt={productShoesData.name}
          className="mx-auto md:mx-0 w-10/12 aspect-auto sticky top-5 my-10"
        />
      </div>
      {/* Product Details */}
      <div className="data-div w-2/4 space-y-3  mx-8 md:mx-0 m-5 text-gray-900 dark:text-gray-100">
        ...
        <div>
            ...
            <p className=" font-normal text-medium uppercase italic">
              {productShoesData.size.map((index) => {
                return (
                  <button
                    key={index}
                    className="py-4 p-6 m-4 rounded-lg bg-gray-300 text-lg  dark:bg-gray-700 "
                  >
                    {index}
                  </button>
                );
          ...
          <div className="w-2/5 gap-2">
            <h6 className="text-base  font-semibold from-stone-800">
              Description:
            </h6>
            <p className=" font-normal text-medium uppercase italic">
