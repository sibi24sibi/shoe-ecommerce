...
function ProductInfo() {
  ...
  const fetchProductData = () => {
    // Check if the _id in products is a string
    const foundProduct = products.find(
      (item) => item._id === productInfoId // Direct comparison without parseInt
    );
              ...
              {productShoesData.size.map((item) => (
                <button
                  ...
                  className={`py-2 px-4 m-2 rounded-lg text-lg ${
                    size === item
                      ? "bg-blue-300 text-black dark:text-white hover:bg-blue-400 dark:bg-blue-600 dark:hover:bg-blue-500"
